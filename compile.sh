...
filename=src/Neptune.AppDir/usr/bin/aisap-0.6.4-alpha-x86_64.AppImage
if [ -f "$filename" ];
then
    :
else
    wget https://github.com/mgord9518/aisap/releases/download/continuous/aisap-0.6.4-alpha-x86_64.AppImage -P src/Neptune.AppDir/usr/bin
    chmod +x src/Neptune.AppDir/usr/bin/aisap-0.6.4-alpha-x86_64.AppImage
fi
